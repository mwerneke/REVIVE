...
def landing_page():
    ...
    with col2:
        ...
        # Display REVIVE logo as text (no file needed)
        st.markdown('<div class="revive-logo">REVIVE</div>', unsafe_allow_html=True)
        ...
        with col_deal:
            st.markdown("""
                <div style='text-align: center; padding: 40px 20px; border: 1px solid #333333; border-radius: 4px;'>
...
def operations_page():
    ...
    with tab1:
        ...
        with col_view:
            ...
            if st.session_state.orders:
                # Display orders with edit/delete options
                for idx, order in enumerate(st.session_state.orders):
                    col1, col2, col3, col4, col5, col_del = st.columns([2, 1, 1, 1.5, 1.2, 0.8])
                    
                    with col1:
                        st.write(f"**{order['customer']}**")
                    with col2:
                        st.write(f"{order['volume']:,}")
                    with col3:
                        st.write(f"${order['price']:.2f}")
                    with col4:
                        # Status selector - updates order in place
                        new_status = st.selectbox(
                            "Status", 
                            ["Pending", "In Production", "Completed", "Delivered", "Paid"],
                            index=["Pending", "In Production", "Completed", "Delivered", "Paid"].index(order['status']),
                            key=f"status_{idx}"
                        )
                        if new_status != order['status']:
                            ...
                            st.rerun()
                    with col5:
                        st.write(f"${order['revenue']:,.0f}")
                    with col_del:
                        if st.button("🗑️", key=f"delete_{idx}", help="Delete order"):
                            st.session_state.orders.pop(idx)
                            st.success("✅ Order deleted")
                            st.rerun()
                
                st.markdown("---")
                st.markdown("<p style='color: #888888; font-size: 11px;'>💡 Click status dropdown to update | 🗑️ to delete</p>", unsafe_allow_html=True)
            else:
    ...
    with tab3:
        ...
        if st.session_state.orders:
            ...
            st.write(f"You have {len(st.session_state.orders)} orders with total revenue of ${total_revenue:,.0f} and profit of ${total_profit:,.0f}")
        else:
    ...
    with tab4:
        ...
        with col_v1:
            st.metric("Year 1 Revenue", "$2.5M - $10M")
        with col_v2:
            st.metric("Year 1 Profit", "$1.5M - $6M")
        with col_v3:
            st.metric("Profit Margin", "60%")
        ...
        with col_val1:
            st.markdown("<h4>Market Fundamentals</h4>", unsafe_allow_html=True)
            st.write("""
- Pickleball growth: +15%/year
- US market: 5M+ players
- Addressable market: 1M+ potential customers
- No major competitors
            """)
        
        with col_val2:
            st.markdown("<h4>Business Strengths</h4>", unsafe_allow_html=True)
            st.write("""
- 60% gross margins (top 5% of mfg)
- Low capital requirements
- Repeat revenue (skins wear out)
- Customizable IP protection
            """)
        
        st.markdown("---")
        
        st.markdown("<h4>Year 3 Valuation</h4>", unsafe_allow_html=True)
